...
void doInference(IExecutionContext& context, cudaStream_t& stream, void **buffers, float* input, float* output, int batchSize) {
    ...
    CUDA_CHECK(cudaMemcpyAsync(buffers[0], input, batchSize * 3 * INPUT_H * INPUT_W * sizeof(float), cudaMemcpyHostToDevice, stream)); //将输入拷贝到buffers
    // 执行推理
    context.enqueue(batchSize, buffers, stream, nullptr); 
    // 最后处理数据在内存实现的,需要将数据由显存读取到内存中
...
int main(int argc, char** argv) {
    ...
    assert(engine != nullptr);
    IExecutionContext* context = engine->createExecutionContext(); //创建推理上下文
    ...
    while(1)
    {
        ...
        doInference(*context, stream, buffers, data, prob, BATCH_SIZE); //prob输出
